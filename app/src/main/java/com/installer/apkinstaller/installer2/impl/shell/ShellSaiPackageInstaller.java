...
import com.installer.apkinstaller.installer2.base.model.AndroidPackageInstallerError;
...
import com.installer.apkinstaller.installer2.base.model.SaiPiSessionStatus;
import com.installer.apkinstaller.installer2.impl.BaseSaiPackageInstaller;
...
public abstract class ShellSaiPackageInstaller extends BaseSaiPackageInstaller {
    ...
    private String getSessionInfo(ApkSource apkSource) {
        ...
        try {
            ...
            Log.wtf(tag(), "Unable to get Installer APK version", e);
        }
        return String.format("%s: %s %s | %s | Android %s | Using %s ApkSource implementation | Installer APK %s", getContext().getString(R.string.installer_device), Build.BRAND, Build.MODEL, MiuiUtils.isMiui() ? "MIUI" : "Not MIUI", Build.VERSION.RELEASE, apkSource.getClass().getSimpleName(), saiVersion);
    }
